...
** 这一段缺少翻译 **
有时，如果你不是小心，创建的可变的值对象将会打破这种规则，我们以`Money`类为例。
...
用这种方式，一旦`Money`被创建，在它的生命周期将保持最初的状态。
...
相比的普通的类实现来说，这种方式要简单很多。属性被声明通过`Struct.new`接口

`hash` and `==`不需要定义，因为他们已经从`Struct`中继承了。

但是用`Struct`定义值对象有些缺点，比如可以通过默认的属性访问器改变他们的属性，属性也可以有默认值
...
为了保持用`Struct`创建值对象的简洁性以及解决它存在问题，我们可以选择`Value`gem, 它类似`Struct`风格，同时也解决以上的所有问题。
下面是这个gem的一个demo：
...
##  总结
这篇文章从值对象地定义开始，展示了从简单地对象到复杂地领域特定对象地用法，同时也深入地了解了值对象在生命周期中地一致性规则
